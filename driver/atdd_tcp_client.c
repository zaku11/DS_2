...
struct atdd_tcp_client * atdd_client_create(struct atdd_client_config config)
{
  ...
  atdd_c->parse_buf = kmalloc(MAX_RESPONSE_SIZE + SECTOR_SIZE, GFP_KERNEL);
  if (!atdd_c->parse_buf) {
    err = -ENOMEM;
    goto err_parse_buf;
  }
  ...
  if ((err = sock_create(PF_INET, SOCK_STREAM, IPPROTO_TCP, &atdd_c->sock)))
    goto err_sock;

  atdd_c->client_state_change_wq = config.client_state_change_wq;
...
int send_atdd_cmd_over_tcp(struct atdd_tcp_client *atdd_c)
{
  int i, err;
  if (!atdd_c->connected) {
    atdd_c->connection_target %= atdd_c->config.addrs_count;
    for (i = 0; i < atdd_c->config.addrs_count; i++) {
      if ((err = tcp_connect(atdd_c->sock, atdd_c->config.atdd_addrs + ((atdd_c->connection_target + i) % atdd_c->config.addrs_count))) == 0) {
...
static int progress_tcp_data_to_limit(struct atdd_tcp_client *atdd_c, size_t limit)
{
  ...
  while (atdd_c->parse_buf_pos < limit) {
    if ((res = try_tcp_read(atdd_c->sock, atdd_c->parse_buf + atdd_c->parse_buf_pos,
        limit - atdd_c->parse_buf_pos)) < 0)
      return res;
    atdd_c->parse_buf_pos += res;
  }
  return 0;
}
